...
import { ITransfer } from "../types/index";
import {
...
export const PolkadotProvider: React.FC<PolkadotProviderProps> = ({
  ...
  async function transfer({ to, amount, callback, gameId }: ITransfer) {
    ...
    // Create a remark extrinsic with the topic Id or any other metadata
    const remark = api.tx.system.remark(stringToHex(`${gameId}`));

    // Create the transfer extrinsic
    const transfer = api.tx.balances.transferKeepAlive(to, amt);

    // Batch the remark and transfer together
    const tx = api.tx.utility.batch([remark, transfer]);
      ...
      try {
        ...
        if (!user?.accessToken) {
          ...
          setUser({
            ...
            playerId: loginResponse.data.user.player_id,
